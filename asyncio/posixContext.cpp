...
struct ReadJob
{
    ...
    bool readData( const FutureJob< AlignedUInt8Vector >&, FutureDataPtr< AlignedUInt8Vector >::type futureData )
    {
...
void addReadJob( const FutureJob<AlignedUInt8Vector>& futureJob, ReadJobPtr readJob )
{
    boost::mutex::scoped_lock( mutex_ );
    jobMap[ futureJob.getJobID() ] = readJob;
}

void removeReadJob( const FutureJob<AlignedUInt8Vector>& futureJob, const FutureDataPtr< AlignedUInt8Vector >::type )
{
    boost::mutex::scoped_lock( mutex_ );
...
FutureDataPtr< AlignedUInt8Vector >::type PosixContext::submitIO( int32_t file,
                                                                  int32_t pos,
                                                                  size_t size )
{
    ...
    FutureJobPtr< AlignedUInt8Vector >::type job =
